...
| Redis 7 | 去重、缓存、Token 黑名单 | 提醒去重、首页统计缓存、退出登录黑名单 |
...
| 首页统计缓存 | `fixledger:dashboard:summary:{familyId}` | 首页热点数据短期缓存或刷新标记 |
...
建议演示时不要从数据库表开始，而是从家庭场景开始：

1. 打开 `http://localhost:5173`，登录 `demo / fixledger123`。
2. 进入“我的家”，讲家庭健康分、本周事项、提醒日历和房间概览。
3. 进入“设备护照”，展示设备卡片和筛选，强调不是普通资产表。
4. 打开“净水器”设备详情，讲设备基础信息、保修、耗材、维修和附件聚合。
5. 进入“家庭日历”或首页日历，讲保修和耗材提醒是定时任务生成，不依赖前端触发。
6. 进入“耗材管理”，演示记录一次滤芯更换，说明会重新计算下次提醒日期。
7. 进入“维修记录”，讲维修状态流转和费用统计规则。
8. 进入“凭证盒”，讲发票、保修卡、说明书、维修单统一归档到 RustFS。
9. 进入“智能助手”，演示故障排查建议，强调 AI 只辅助不自动改数据。
10. 打开 Swagger 或 README，说明接口、Docker、测试和后续计划。
...
- 已做 RustFS 和 CI，因为它们直接提升面试演示完整度和工程可信度。
- 暂缓真实 AI、OCR、邮件/Webhook、附件在线预览和 Refresh Token，因为它们会引入外部服务、密钥、回调、预览安全和更复杂的登录续期策略，不是家庭设备生命周期 MVP 的阻塞项。
- 后续会把这些增强放入 P10+ 或 P15 继续推进，保证当前项目边界清晰、可运行、可解释。

后续建议：

- P10：继续做深度文档对齐和接口/数据库复核。
- 家庭成员邀请、移除、角色调整。
- 默认设备分类自动初始化。
- 附件预览和对象存储临时 URL。
- 真实 AI Provider、OCR、邮件/Webhook 通知。
- 前端 E2E 和更完整的 CI/CD。
