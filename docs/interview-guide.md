...
> 我做的是一个家庭设备管家系统，不是企业资产后台。它围绕一台设备从购买、上传发票、记录保修、配置耗材周期、生成提醒、记录维修、归档凭证到导出家庭设备和维修费用数据的完整生命周期展开。技术上使用 Spring Boot 3 + JDK 21 + MyBatis Plus + MySQL + Redis + RustFS + Vue3，重点体现分层设计、数据隔离、定时任务、文件存储、AI 解耦、操作日志和 Docker 一键启动。
...
| 系统审计 | `sys_operation_log` | 家庭协作等关键操作日志 |
...
| 操作日志 | `GET /api/system/operation-logs` |
| 设备清单导出 | `GET /api/families/{familyId}/exports/devices.csv` |
| 维修费用导出 | `GET /api/families/{familyId}/exports/maintenance-costs.csv` |
...
- 系统：家庭协作操作日志分页和家庭权限过滤。
- 导出：设备清单、维修费用口径、越权访问和 CSV 安全转义。
