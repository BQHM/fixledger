...
截至 P24，项目已经从“能运行”推进到“能演示、能解释、能验证，并具备家庭协作、基础生产门禁和家庭数据出口”的状态：
...
- 真实 AI Provider、真实邮件/Webhook 投递、OCR、复杂 PDF 解析、对象存储临时 URL、Refresh Token、生产域名和云端发布仍作为后续增强。

### 推荐后续开发路线

- P25 设备二维码标签：为设备生成二维码标签，扫码进入设备详情或只读设备卡片，适合贴在路由器、净水器、洗衣机等实物上。
- P26 OCR 与智能归档：补充发票图片/说明书 PDF 文字识别，把 AI 继续限定在“生成草稿、用户确认”的辅助录入边界内。
- P27 移动端与 PWA：优化手机端布局、触控操作和弱网体验，让家庭成员现场查看设备档案和凭证更顺手。
- P28 外部通知渠道：在现有 `NotificationService` 后扩展邮件/Webhook 投递、失败记录和重试策略，不扩大核心事务。
- P29 性能与可观测性：补首页热点缓存、慢查询审查、关键接口日志指标和导出异步化预案。
