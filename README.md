...
| `docs/demo-guide.md` | P14 演示指南：一键启动、演示账号、数据地图、5-10 分钟路线和排障预案 |
...
面试时可以说明：CI 的作用是把本地验证固化成仓库级自动检查，避免“只在我电脑能跑”的问题。当前凭证盒图片/PDF 预览走前端 Blob 预览层，P16.2 已补充说明书第一版关键词搜索；真实部署、邮件/Webhook、OCR、复杂 PDF 解析、对象存储临时 URL 和 Refresh Token 属于后续增强。
...
截至 P16.2，项目已经从“能运行”推进到“能演示、能解释、能验证，并具备更完整的家庭设备产品体验”：
...
- P15 产品体验升级已完成：公开首页、米家风格视觉基线、设备护照深化和凭证盒图片/PDF 预览已完成。
- P16 凭证聚合增强已推进：凭证盒后端聚合接口和说明书第一版关键词搜索已完成；真实 AI Provider、邮件/Webhook、操作日志、OCR、复杂 PDF 解析和更完整 CI/CD 部署仍作为后续增强。
